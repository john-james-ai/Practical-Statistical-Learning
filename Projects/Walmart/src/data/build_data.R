...
train_raw <- readr::read_csv(unz('data/external/train.csv.zip', 'train.csv'))
...
readr::write_csv(train, 'data/raw/train_ini.csv')

# create test.csv 
# removes weekly sales
test %>% 
  select(-Weekly_Sales) %>% 
  readr::write_csv('data/raw/test.csv')
...
for (i in 1:num_folds) {
    ...
    readr::write_csv(test_fold, paste0('data/cv/fold_', i, '.csv'))
}